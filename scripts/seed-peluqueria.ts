...
async function main() {
  const existing = await db.prepare("SELECT id FROM tenants WHERE slug = ?").get("style-demo");
  if (existing) {
    console.log("La tienda style-demo ya existe. Eliminando para recrear...");
    const t = existing as { id: string };
    await db.prepare("DELETE FROM staff_schedules WHERE tenant_id = ?").run(t.id);
    await db.prepare("DELETE FROM staff WHERE tenant_id = ?").run(t.id);
    await db.prepare("DELETE FROM appointments WHERE tenant_id = ?").run(t.id);
    await db.prepare("DELETE FROM products WHERE tenant_id = ?").run(t.id);
    await db.prepare("DELETE FROM categories WHERE tenant_id = ?").run(t.id);
    await db.prepare("DELETE FROM payment_methods WHERE tenant_id = ?").run(t.id);
    await db.prepare("DELETE FROM users WHERE tenant_id = ?").run(t.id);
    await db.prepare("DELETE FROM tenants WHERE id = ?").run(t.id);
  }
  ...
  ).run(newId(), "style@catalogo-demo.test", passwordHash, "Dueño Style", "OWNER", tenantId, ts);
  ...
  // Coupon
  await db.prepare(`INSERT INTO coupons (id, tenant_id, code, type, value, created_at) VALUES (?, ?, ?, ?, ?, ?)`).run(
  ...
  for (const s of staff) {
    const staffId = newId();
    await db.prepare(`INSERT INTO staff (id, tenant_id, name, active, created_at) VALUES (?, ?, ?, 1, ?)`).run(
  ...
  console.log("  Login admin: style@catalogo-demo.test / style1234");
